...
public class Professor extends Person implements Validatable {
    ...
    public void validate() throws InvalidInputException {
        if (getName() == null || getName().trim().isEmpty()) {
            throw new InvalidInputException("Professor name cannot be empty");
        }
        if (getAge() < 21) {
            throw new InvalidInputException("Professor must be at least 21 years old");
        }
        if (subject == null || subject.trim().isEmpty()) {
            throw new InvalidInputException("Professor must teach a subject");
        }
        if (salary <= 0) {
            throw new InvalidInputException("Professor salary must be positive");
        }
    }
